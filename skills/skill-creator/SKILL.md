...
- "Where should I create this skill? If you do not have a preference, I will place it in `<vault-root>/Oliver/_Project Repos/oliver-local/skills` so shared runtime adapters can discover it consistently."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `<vault-root>/Oliver/_Project Repos/oliver-local/skills` so shared runtime adapters can discover it consistently.
...
```bash
scripts/init_skill.py my-skill --path "$HOME/oliver-local/skills"
scripts/init_skill.py my-skill --path "$HOME/oliver-local/skills" --resources scripts,references
