...
`<vault-root>/Oliver/_Project Repos/oliver-local` is the shared orchestration system root for this machine.
...
Those files explain what each local runtime folder is, what state may live there, and why shared instructions point back to `<vault-root>/Oliver/_Project Repos/oliver-local`. `scripts/test-structure.sh` checks that the live runtime files match these tracked copies.
...
- Shared skills live in `<vault-root>/Oliver/_Project Repos/oliver-local/skills/*`.
