...
Canonical shared orchestration files live in `<vault-root>/_Management/Agent Orchestration/repos/oliver-local`. Do not treat `~/.claude` as the shared-system source of truth.
...
- `~/.claude/skills` points to `<vault-root>/_Management/Agent Orchestration/repos/oliver-local/skills`.
