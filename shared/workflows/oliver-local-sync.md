...
`oliver-local` is the shared orchestration repo at `<vault-root>/Oliver/_Project Repos/oliver-local`, not an app repo.
...
```bash
git -C <vault-root>/Oliver/_Project Repos/oliver-local status --short --branch
test -d <vault-root>/Oliver/_Project Repos/oliver-local
```
...
jq -r '.repos[].name + " -> " + .path' <vault-root>/Oliver/_Project Repos/oliver-local/shared/repo-map.json
```
...
```bash
git -C <vault-root>/Oliver/_Project Repos/oliver-local add shared/repo-map.json docs README.md CLAUDE.md shared/workflow-standards.md
git -C <vault-root>/Oliver/_Project Repos/oliver-local commit -m "docs: sync oliver-local repo map and workflow metadata"
```

7. Push `main` immediately.

```bash
git -C <vault-root>/Oliver/_Project Repos/oliver-local push origin main
```
